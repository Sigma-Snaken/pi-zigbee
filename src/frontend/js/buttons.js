...
export async function initButtons(ws) {
    ...
    ws.on('button:activity', (data) => onActivity(data));
    await renderButtons();
}

function onActivity(data) {
    const ieee = data.ieee_addr;
    const row = container.querySelector(`tr[data-ieee="${ieee}"]`);
    if (!row) return;

    // Update last_seen cell
    if (data.last_seen) {
        const cell = row.querySelector('.last-seen');
        if (cell) cell.textContent = new Date(data.last_seen).toLocaleString('zh-TW', {hour12: false});
    }
    // Update battery cell
    if (data.battery != null) {
        const cell = row.querySelector('.battery');
        if (cell) cell.textContent = data.battery + '%';
    }
